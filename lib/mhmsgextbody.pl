...
ATYPE: {
        ...
        if (   $access_type eq 'ftp'
            ...
            my $proto =
                ($access_type eq 'x-http' || $access_type eq 'http') ? 'http'
                : ($access_type eq 'tftp') ? 'tftp'
                :                            'ftp';
            my $url =
                  $proto . '://'
            ...
            $ret .=
                qq|<a rel="nofollow" href="$url">&lt;$url&gt;</a></dt><dd>\n|;
        ...
        if ($access_type eq 'local-file') {
            ...
            $ret .= qq|Expires: <tt>$expires</tt><br>\n|
                if $expires;
            $ret .=
                  qq|File accessible from the following domain: |
                . qq|$site<br>\n|
                if $site;
            $ret .= "</dd></dl>\n";
            last ATYPE;
